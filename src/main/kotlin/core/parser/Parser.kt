...
class Parser(tokens: Iterable<Token>) {
    ...
    private fun parseVarDeclaration(): Statement {
        ...
        var typeAnnotation: TokenType? = null
        ...
        return VarStatement(name.value, typeAnnotation, initializer)
    }

    private fun parseTypeAnnotation(): TokenType {
        if (match(TokenType.TYPE_NUMBER)) return TokenType.TYPE_NUMBER
        if (match(TokenType.TYPE_STRING)) return TokenType.TYPE_STRING
        if (match(TokenType.TYPE_BOOLEAN)) return TokenType.TYPE_BOOLEAN
        val token = peek()
        throw Exception("[Parser Error] Line ${token.line}, Col ${token.column}: Expected type name (Number, String, Boolean).")
    }
