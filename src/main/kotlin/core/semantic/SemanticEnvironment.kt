...
class SemanticEnvironment(private val parent: SemanticEnvironment? = null) {
    ...
    fun defineVariable(name: String, type: ValueType, isInitialized: Boolean): Boolean {
        if (variables.containsKey(name)) {
            return false
        }

        variables[name] = VariableInfo(type = type, isInitialized = isInitialized)
        return true
    }
    ...
    fun getVariableType(name: String): ValueType? {
        val variableInfo = variables[name]
        if (variableInfo != null) {
